...
- **服務層（Services）**：位於 `services/` 資料夾，封裝核心業務邏輯功能。包含 **AI 核心**、**語音轉文字服務 (STT)**、**文字轉語音服務 (TTS)**、**情緒分析服務**等模組。其中 **AI 核心** 是重點：
    ...
    - *EmotionAnalyzer*（情緒分析服務）：**（目前較少使用）** 分析使用者文字中的情緒傾向，推斷表情與意圖。主要情緒表達由 `DialogueGraph` 決定。
...
graph TD
    ...
    subgraph 後端服務層
        ...
        %% AnimationService REMOVED
    end

    %% 工具與設定
    subgraph 工具與設定
        Logger["日誌模組"]
        Constants["常數定義 (部分可能過時)"]
        Config["應用設定"]
        Models["資料模型"]
    end
    ...
    REST_API -- (獲取預設表情) --> PresetAPI["預設表情接口"]
    REST_API -- 調用 --> AIService["(STT完成後)"]
    REST_API -- (健康檢查) --> HealthCheck["健康檢查接口"]

    WebSocket -- 接收用戶消息 / 發送 chat-message, emotionalTrajectory --> AIService
    ...
    %% DialogueGraph handles emotion/animation data generation
    %% TextToSpeech provides duration for emotionalTrajectory
    ...
    %% Constants usage might be reduced or changed
    REST_API --> Models
    WebSocket --> Models
    DialogueGraph --> Models
    DialogueGraph --> Config
