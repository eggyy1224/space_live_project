...
- [ ] **集中動畫配置管理:**
  * 創建一個集中式的動畫配置文件 `animationConfig.ts`，作為單一真相來源。
  * 將動畫名稱、路徑、描述整合到一個結構化對象中。
  * 替換 `modelConfig.ts` 中的 `EXTERNAL_ANIMATION_PATHS` 為從新配置生成的數組。

- [ ] **實現前後端配置自動同步:**
  * 建立機制將前端動畫配置自動同步到後端。
  * 添加構建步驟生成 `animationNames.json` 供後端讀取。
  * 修改 `dialogue_graph.py` 在啟動時動態讀取前端配置，而不是硬編碼 `ALLOWED_ANIMATION_NAMES`。

- [ ] **增強動畫描述系統:**
  * 為每個動畫添加詳細描述，幫助 LLM 更好地理解和選擇合適的動畫。
  * 在提示中包含動畫描述，讓生成的動畫序列更符合對話內容。
  * 根據情緒和動作語義建立動畫推薦系統。

- [ ] **改進後端驗證機制:**
  * 實現更智能的檢查以識別未使用的可用動畫。
  * 添加失敗安全機制，確保即使某些動畫不可用也能降級使用其他動畫。
  * 添加指標收集，記錄動畫使用頻率和效果評估。