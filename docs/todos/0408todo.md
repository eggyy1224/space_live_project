...
*   [ ] **定義情緒基礎定義 (`Emotion Base Definitions`):**
...
*   [ ] **定義中心計算點 (實現多情緒插值):**
    ...
    *   此 Hook 需要：
        ...
        *   在每一幀 (`useFrame` 內部或由其觸發)：
            ...
            *   獲取 `kf_prev.tag` 和 `kf_next.tag` 的基礎權重 `weights_prev`, `weights_next`。
            *   **執行插值：** 計算當前幀的情緒目標權重 `currentEmotionWeights` (對所有相關 Blendshape 進行 `lerp(weights_prev[key], weights_next[key], localProgress)`)。
*   [ ] **實現說話狀態管理 (`Speech State Management Layer`):**
    *   實現一個簡單函數，根據 `isSpeaking` 狀態輸出基礎的說話 Blendshape 目標權重 (`speechBlendshapeWeights`)。
    *   例如：`if (isSpeaking) { return { jawOpen: 0.7 }; } else { return { jawOpen: 0.0, mouthClose: 0.8 }; }` （數值需調整）。
*   [ ] **實現 Blendshape 合併與混合策略 (`Blendshape Mixing Layer`):**
