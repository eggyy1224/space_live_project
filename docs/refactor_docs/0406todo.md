# 0406 前端重構 TODO List

本文檔基於對專案現狀的分析和未來規劃，旨在指導前端部分的漸進式重構，以構建一個穩定、高效、可擴展且能生動體現 AI 角色特質的前端系統。

參考文件：
- [專案總覽與願景 (README.md)](../../README.md)
- [現有前端架構分析 (docs/前端相關/0402前端架構.md)](../前端相關/0402前端架構.md)
- [前端效能問題分析 (docs/refactor_docs/0402前端效能問題分析與優化建議.md)](./0402前端效能問題分析與優化建議.md)
- [前端重構計劃 (docs/refactor_docs/0405前端重構計劃.md)](./0405前端重構計劃.md)

---

## 路線一：穩固陣地，提升即時互動體驗

**目標：** 快速解決影響使用者體驗的效能瓶頸，確保基礎互動流暢。

**參考：**
- [效能問題分析](./0402前端效能問題分析與優化建議.md)
- [重構計劃第一期](./0405前端重構計劃.md#:~:text=第一期：即時效能提升與基礎重構)

**待辦事項：**

-   [x] **記憶體洩漏修復：**
    -   [x] 確保 WebSocket 連線在組件卸載時正確調用 `disconnect()` ([參考](./0402前端效能問題分析與優化建議.md#:~:text=WebSocket%20連線未適時關閉))。
    -   [x] 實現 3D 模型切換時，舊模型資源 (geometry, material, texture) 的 `dispose()` ([參考](./0402前端效能問題分析與優化建議.md#:~:text=3D%20模型資源未釋放))。 *(註：已加入 dispose 邏輯，但存在 linter 誤報，需手動確認)*
-   [x] **減少不必要的 React 重繪：**
    -   [x] 將聊天列表的 `key` 從索引改為唯一 ID ([參考](./0402前端效能問題分析與優化建議.md#:~:text=列表項目使用不穩定的%20key))。
    -   [x] 優化 Morph Target 更新機制，減少經由 React `setState` 的頻率 ([參考](./0402前端效能問題分析與優化建議.md#:~:text=Morph%20Target%20狀態頻繁更新))。
    -   [x] 分析 `ControlPanel` 和 `ModelViewer` 的 props 傳遞，使用 `React.memo` 和 `useCallback` 優化 ([參考](./0405前端重構計劃.md#:~:text=減少不必要的%20React%20重繪))。
    -   [x] 檢查 `useAudioService` 和 `useChatService` 返回狀態的引用穩定性，並優化 `useChatService` 中的 `emotion` 更新邏輯 ([參考](./0405前端重構計劃.md#:~:text=減少不必要的%20React%20重繪))。
-   [ ] **渲染迴圈優化：**
    -   [ ] 將高頻動畫更新 (如嘴型同步插值、表情平滑過渡) 移入 R3F 的 `useFrame` Hook 處理 ([參考](./0405前端重構計劃.md#:~:text=使用%20useFrame%20Hook))。
    -   [ ] 研究並視情況使用 R3F 的 `frameloop="demand"` 或 `invalidate()` 來控制閒置時的渲染 ([參考](./0405前端重構計劃.md#:~:text=暫停未使用的動畫迴圈))。
-   [ ] **動畫與計時器管理：**
    -   [ ] 確保 WebSocket 重連計時器在成功連線後被清除 ([參考](./0402前端效能問題分析與優化建議.md#:~:text=WebSocket%20重連計時器))。
    -   [ ] 確保本地備用嘴型動畫 (Interval) 在接收到即時 Lipsync 數據時暫停 ([參考](./0402前端效能問題分析與優化建議.md#:~:text=過多同時進行的動畫))。
-   [ ] **基礎資源管理：**
    -   [ ] 審查 3D 資源加載，確保使用 `useMemo` 或 R3F Loader 快取機制，避免重複創建 ([參考](./0405前端重構計劃.md#:~:text=物件重用與記憶體優化))。

---

## 路線二：重塑骨架，奠定可擴展架構

**目標：** 引入現代化狀態管理，重構組件與服務，建立清晰、低耦合的架構基礎。

**參考：**
- [現有前端架構](../前端相關/0402前端架構.md)
- [重構計劃第二期](./0405前端重構計劃.md#:~:text=第二期：代碼模組化與維護性改進)
- [重構計劃第三期](./0405前端重構計劃.md#:~:text=第三期：引入集中式狀態管理與架構優化)

**待辦事項：**

-   [ ] **引入 Zustand 狀態管理：**
    -   [ ] 添加 Zustand 依賴。
    -   [ ] 創建全局 Store，定義核心狀態切片 (slices)，如 `useAppStore`, `useAvatarStore` ([參考](./0405前端重構計劃.md#:~:text=導入%20Zustand%20作為全域狀態儲存))。
    -   [ ] 逐步將原有 Context 或服務管理的全局狀態遷移至 Zustand。
-   [ ] **組件拆分與模組化：**
    -   [ ] 拆分大型組件，如將 `App.tsx` 中的 3D 場景 (`<AvatarScene>`) 和控制面板 (`<ControlPanel>`) 抽離 ([參考](./0405前端重構計劃.md#:~:text=拆分大型組件))。
    -   [ ] 審查 `src/` 目錄結構，確保模組職責單一 ([參考](./0405前端重構計劃.md#:~:text=整理目錄結構))。
-   [ ] **服務層解耦：**
    -   [ ] 調整服務 (WebSocket, Chat, Audio, Model) 與狀態的互動模式，使其透過更新 Zustand store 來觸發前端變化，而非直接調用其他模組 ([參考](./0405前端重構計劃.md#:~:text=調整服務層與狀態的互動))。
    -   [ ] 考慮服務實例的提供方式 (Context 或依賴注入)，降低直接 import 全局單例的耦合度 ([參考](./0405前端重構計劃.md#:~:text=服務層單元化))。
-   [ ] **建立清晰分層：**
    -   [ ] 明確 UI 層、狀態層 (Zustand)、服務/控制層的職責與交互方式 ([參考](./0405前端重構計劃.md#:~:text=引入更佳架構模式))。
-   [ ] **類型與註解：**
    -   [ ] 在重構過程中，確保 TypeScript 類型定義清晰，並為新模組添加必要的 JSDoc 註解 ([參考](./0405前端重構計劃.md#:~:text=增強類型與文件註解))。

### 2.4 引入狀態管理（Zustand）的基礎準備
- [ ] 確保組件之間共享狀態的介面清晰。
- [ ] 考慮將某些全域狀態轉移到 React Context 或 hook (作為過渡)。

### 2.5 增強類型與文件註解
- [ ] 為重構後的模組補充清晰的 TypeScript 型別定義。
- [ ] 為主要模組添加必要的註解。

### 2.6 其他代碼清理與 Bug 修復 (第二期)
- [X] 修復 `App.tsx` 中移除 `morphTargetInfluences` 後引入的 Linter 錯誤 (涉及 `ModelViewer`, `Model`)。
- [X] 修正 `Model.tsx` 中 `logger.info`/`logger.debug` 的參數類型錯誤。
- [X] 解決模型切換後新模型不播放動畫的問題：
    - [X] `ModelService` 添加 `switchModel` 方法，並在切換時重置相關狀態。
    - [X] `Model.tsx` 確保 `key={url}` 存在，並優化預設動畫播放邏輯 (嘗試 Idle 或第一個可用動畫)。
    - [X] 修復 `ModelService` 中的 Linter 錯誤 (重複回調)。
- [ ] ... (其他待發現或待處理的清理任務)

## 第三期：引入集中式狀態管理與架構優化 (Phase 3 Integration)

**目標：** 導入 Zustand 管理應用狀態，優化架構，為「頭部表情」與「身體動畫」獨立控制做準備。

### 3.1 導入 Zustand
- [ ] 新增 Zustand 並建立全局 store (`useAppStore`, `useAvatarStore` 等)。
- [ ] 定義狀態切片 (slices) 和 actions。
- [ ] 逐步將 Context 或 service 中的狀態遷移到 Zustand store。

### 第三期：引入 Zustand 狀態管理與架構優化 (Phase 3 Integration)

- [ ] **1. 設計並定義 Zustand Store 結構**
    - [ ] **識別全局狀態需求**:
        - `AppSlice`: `activeTab`, `isDebugMode`, `isCameraFar`, `showSpaceBackground`
        - `WebSocketSlice`: `isConnected`
        - `ChatSlice`: `messages`, `isProcessing`, `currentEmotion` (結構化: `{ emotion: string; confidence: number; }`)
        - `ModelSlice`: `modelUrl`, `modelLoaded`, `modelScale`, `modelRotation`, `modelPosition`, `morphTargets`, `availableAnimations`, `currentAnimation`
        - `AudioSlice`: `isRecording`, `isPlaying`, `currentAudioSource` (新增)
    - [ ] **規劃 Store Slices 接口 (TypeScript Interfaces)**:
        ```typescript
        // 示例接口結構 (具體在 slice 文件中實現)
        interface AppSlice {
          activeTab: string; isDebugMode: boolean; /*...*/
          setActiveTab: (tab: string) => void; toggleDebugMode: () => void; /*...*/
        }
        interface WebSocketSlice { /*...*/ }
        interface ChatSlice { /*...*/ }
        interface ModelSlice { /*...*/ }
        interface AudioSlice { /*...*/ }
        ```
    - [ ] **定義明確的 Actions**:
        - 使用如 `toggleDebugMode`, `addMessage`, `setEmotion`, `setModelTransform` (批次更新) 等具體 Action 名稱。
        - 考慮 `morphTargets` 的更新策略（淺比較或 debounce）。
- [ ] **2. 安裝 Zustand 並創建 Store 文件結構**
    - [ ] `npm install zustand`
    - [ ] 創建 `src/store/index.ts` (主 Store 文件)
        ```typescript
        // src/store/index.ts 結構示例
        import { create } from 'zustand';
        import { AppSlice, createAppSlice } from './slices/appSlice';
        // ... import 其他 slices ...

        type Store = AppSlice & WebSocketSlice & ChatSlice & ModelSlice & AudioSlice;

        export const useStore = create<Store>((...args) => ({
          ...createAppSlice(...args),
          // ... 組合其他 slices ...
        }));
        ```
    - [ ] 創建 `src/store/slices/` 目錄
    - [ ] 在 `slices/` 下創建獨立的 slice 文件 (`appSlice.ts`, `webSocketSlice.ts`, 等)
        ```typescript
        // slices/appSlice.ts 結構示例
        import { StateCreator } from 'zustand';
        // 可能需要從 store/index.ts 導入組合後的 Store 類型以支持跨 slice 操作

        export interface AppSlice { /* State and Action types */ }

        export const createAppSlice: StateCreator<AppSlice /* possibly intersected with other slices if needed */> = (set) => ({
          // initial state...
          activeTab: 'chat',
          isDebugMode: false,
          // actions...
          setActiveTab: (tab) => set({ activeTab: tab }),
          toggleDebugMode: () => set(state => ({ isDebugMode: !state.isDebugMode })),
          // ... 其他 state 和 actions
        });
        ```
- [ ] **3. 逐步遷移狀態 (建議順序：從簡單到複雜)**
    - [ ] **遷移 WebSocketSlice (`isConnected`)**:
        - [ ] 實現 `webSocketSlice.ts`。
        - [ ] 修改 `WebSocketService` 使用 Store Action 更新 `isConnected`。
        - [ ] 修改 `App.tsx` (或相關 Hook) 從 Store 讀取 `isConnected`。
        - [ ] 移除 `WebSocketService` 舊的回調邏輯。
        - [ ] **測試驗證**: WebSocket 連接狀態是否正確反映在 UI 上。
    - [ ] **遷移 AudioSlice (`isRecording`, `isPlaying`, `currentAudioSource`)**:
        - [ ] 實現 `audioSlice.ts`。
        - [ ] 修改 `AudioService` 使用 Store Actions。
        - [ ] 修改 `AudioControls` (或相關 Hook) 從 Store 讀取狀態並調用 Actions。
        - [ ] 移除 `AudioService` 舊的回調邏輯。
        - [ ] **測試驗證**: 錄音/播放狀態和按鈕是否同步。
    - [ ] **遷移 AppSlice (UI 相關狀態)**:
        - [ ] 實現 `appSlice.ts`。
        - [ ] 修改 `App.tsx` (或 `AppUI`) 將本地狀態移至 Store，並使用 Store 讀取/更新。
        - [ ] **測試驗證**: Debug 模式切換、相機距離、背景切換等 UI 控制是否正常。
    - [ ] **遷移 ChatSlice (`messages`, `isProcessing`, `currentEmotion`)**:
        - [ ] 實現 `chatSlice.ts`。
        - [ ] 修改 `ChatService` 使用 Store Actions。
        - [ ] 修改 `WebSocketService` 的消息處理直接更新 Store。
        - [ ] 修改 `useChatService` Hook (或 `ChatInterface`) 從 Store 讀取/更新。
        - [ ] 移除 `ChatService` 舊的狀態和回調。
        - [ ] **測試驗證**: 聊天記錄顯示、發送消息、機器人回應、情緒顯示是否正常。
    - [ ] **遷移 ModelSlice (模型相關狀態)**:
        - [ ] 實現 `modelSlice.ts`。
        - [ ] 修改 `ModelService` 使用 Store Actions (特別注意 `setMorphTargets` 的性能)。
        - [ ] 修改 `WebSocketService` 的消息處理直接更新 Store。
        - [ ] 修改 `useModelService` Hook (或 `ModelViewer`/`Model`) 從 Store 讀取/更新。
        - [ ] 移除 `ModelService` 舊的狀態和回調。
        - [ ] **測試驗證**: 模型切換、加載、變換、動畫播放、表情/唇同步是否正常。
- [ ] **4. 清理與最終驗證**
    - [ ] 移除所有 Service 中遺留的內部狀態管理和回調代碼。
    - [ ] 確認所有相關狀態均由 Zustand 管理。
    - [ ] 執行全面的功能回歸測試和壓力測試。

## 路線三：賦予靈魂，解耦精細動畫控制

**目標：** 分離頭部/表情與身體動畫控制，實現更細膩、靈活的角色表現。

**參考：**
- [重構計劃第四期](./0405前端重構計劃.md#:~:text=第四期：頭部表情與身體動畫控制拆分)

**待辦事項：**

-   [ ] **設計動畫控制器接口：**
    -   [ ] 定義 `HeadAnimationController` 的接口 (如 `setExpression`, `update`)。
    -   [ ] 定義 `BodyAnimationController` 的接口 (如 `playAnimation`, `update`) ([參考](./0405前端重構計劃.md#:~:text=設計動畫控制模組接口))。
-   [ ] **實現動畫控制器：**
    -   [ ] 實作 `HeadAnimationController`，處理 Morph Target 控制、表情平滑過渡 ([參考](./0405前端重構計劃.md#:~:text=表情控制器實作細節))。
    -   [ ] 實作 `BodyAnimationController`，處理骨骼動畫播放、混合 (可利用 `AnimationMixer` 或 Drei 的 `useAnimations`) ([參考](./0405前端重構計劃.md#:~:text=身體控制器實作細節))。
-   [ ] **整合控制器：**
    -   [ ] 修改 `ModelService` 或相關 3D 組件，初始化並使用這兩個控制器 ([參考](./0405前端重構計劃.md#:~:text=集成控制器至主流程))。
    -   [ ] 在渲染迴圈 (`useFrame`) 中調用兩個控制器的 `update` 方法。
-   [ ] **狀態驅動動畫：**
    -   [ ] 讓控制器監聽 Zustand store 中的相關狀態 (如 `currentEmotion`, `targetAnimation`)。
    -   [ ] 確保狀態變化能正確觸發對應的控制器方法 ([參考](./0405前端重構計劃.md#:~:text=與狀態管理整合))。
-   [ ] **協調與測試：**
    -   [ ] 測試表情和身體動畫同時進行的效果，確保協調自然 ([參考](./0405前端重構計劃.md#:~:text=表情與動作協調處理))。

## 路線四：精兵簡政，保障長期發展

**目標：** 清理技術負債，統一規範，完善文檔與測試，確保專案長期健康。

**參考：**
- [重構計劃第五期](./0405前端重構計劃.md#:~:text=第五期：技術負債清理與最終優化)
- [專案總覽與願景 (README.md)](../../README.md)

**待辦事項：**

-   [ ] **代碼清理：**
    -   [ ] 移除因重構產生的冗餘代碼、註解、舊的狀態管理方式 (如 Context) ([參考](./0405前端重構計劃.md#:~:text=移除冗餘代碼與註解))。
    -   [ ] 移除不再使用的 `console.log`。
-   [ ] **規範統一：**
    -   [ ] 執行 ESLint 和 Prettier，確保所有程式碼風格一致 ([參考](./0405前端重構計劃.md#:~:text=統一代碼風格))。
    -   [ ] 檢查並統一命名規範。
-   [ ] **依賴管理：**
    -   [ ] 審查 `package.json`，更新核心依賴 (React, R3F, Zustand 等) 到穩定新版本 ([參考](./0405前端重構計劃.md#:~:text=更新依賴與版本))。
    -   [ ] 移除未使用的依賴。
-   [ ] **構建與加載優化：**
    -   [ ] 檢查 3D 模型大小，考慮壓縮 (Draco) 或優化 ([參考](./0405前端重構計劃.md#:~:text=優化構建與加載))。
    -   [ ] 利用 Vite 的代碼分割優化初始加載時間。
    -   [ ] 使用 Bundle Analyzer 分析打包結果。
-   [ ] **健壯性提升：**
    -   [ ] 審查並完善錯誤處理邏輯 (API, WebSocket) ([參考](./0405前端重構計劃.md#:~:text=強化錯誤處理與日誌))。
    -   [ ] 在關鍵流程添加必要的日誌記錄。
-   [ ] **文檔更新：**
    -   [ ] 更新 [README.md](../../README.md) 中的架構圖和說明，反映重構後的狀態。
    -   [ ] 為新的核心模組 (如控制器、Zustand store) 補充文檔說明 ([參考](./0405前端重構計劃.md#:~:text=文件維護))。
-   [ ] **(可選) 測試覆蓋：**
    -   [ ] 為核心狀態邏輯 (Zustand actions/reducers) 編寫單元測試 ([參考](./0405前端重構計劃.md#:~:text=編寫測試))。
    -   [ ] 考慮為關鍵交互流程編寫整合測試。
-   [ ] **最終驗收：**
    -   [ ] 進行全面的功能回歸測試。
    -   [ ] 進行效能和壓力測試，確保達到預期目標 ([參考](./0405前端重構計劃.md#:~:text=性能回歸檢查))。

- [ ] **減少不必要的 React 重繪 (次要):** 檢查其他高頻更新是否可以移出 React state。
    - [ ] 分析 `AudioService` 中的狀態更新 (`isRecording`, `isSpeaking`, `isProcessing`) 是否觸發過多重繪，考慮是否能用 ref 或其他方式優化 (可能不需要，優先級低)。

- [ ] **實現基礎的錯誤處理和日誌記錄:**
    - [ ] 在WebSocket連接、消息發送/接收、API調用等關鍵點添加日誌記錄。
    - [ ] 考慮添加一個簡單的錯誤邊界組件 (Error Boundary)。

- [ ] **程式碼清理和註釋 (持續進行):**
    - [ ] 移除不再使用的程式碼片段。
    - [ ] 添加或更新必要的註釋，解釋複雜邏輯。

- [ ] **最終測試和驗證:**

### 5.6 編寫測試
- [ ] 為核心功能撰寫單元測試或整合測試 (狀態管理、控制器、服務交互)。

### 5.7 其他代碼清理與 Bug 修復 (第五期)
- [ ] 審查並移除所有不再使用的代碼、註解、日誌。
- [ ] 解決所有 ESLint/TypeScript 警告。
- [ ] ... (其他待發現或待處理的清理任務)

## 第二期：代碼模組化與維護性改進

**目標：** 重構專案結構，提升可讀性與可維護性。

### 2.1 整理目錄結構
- [ ] 審視 `src/` 下各模組職責，確保單一性。
- [ ] 如有必要，調整文件位置或創建新目錄。

### 2.2 拆分大型組件
- [/] **拆分 App.tsx**: 將 UI 佈局與 3D 場景渲染分離。
    - [X] 創建 `<SceneContainer />` 並遷移 3D 渲染邏輯。
    - [X] 創建 `<AppUI />` 並遷移 2D UI 佈局 (AudioControls, Panel/Chat, Debug Buttons)。
    - [X] 修改 `App.tsx` 使用新組件，傳遞 props。
- [ ] 拆分 ControlPanel.tsx (如果內部邏輯過於複雜)。
- [ ] 拆分 ModelViewer.tsx / Model.tsx (如果需要更細粒度控制)。

### 2.3 服務層單元化與接口調整
- [ ] 審查服務單例模式，考慮依賴注入或 Context 提供方式。
- [ ] 調整服務接口，使其更獨立，減少對 UI/3D 組件內部狀態的直接操作。
- [ ] 考慮 WebSocketService 的事件發派模式改造。

---

## 暫停 Zustand 遷移與現狀評估 (2024-04-06)

鑑於引入 Zustand 的嘗試遇到阻礙，我們決定暫停第三期（引入 Zustand）的重構工作，並對當前狀況進行評估。

### 1. 目前進度與架構

- **已完成**: 
    - `App.tsx` 拆分：將 UI 佈局 (`<AppUI />`) 與 3D 場景 (`<SceneContainer />`) 分離。
    - 部分代碼清理和 Bug 修復 (如 Linter 錯誤、模型動畫播放問題等)。
- **當前架構**: 
    - UI 由 React 組件構成。
    - 核心業務邏輯和共享狀態主要由單例服務 (`ModelService`, `ChatService`, `AudioService`, `WebSocketService`) 管理。
    - 組件/Hooks 通過直接調用服務方法或訂閱服務提供的回調函數 (`on...UpdateCallbacks`) 來與服務交互和獲取狀態更新。
    - 狀態管理相對分散，散佈在各個服務內部和部分組件的本地狀態中。

### 2. 現有架構擴充性評估

- **短期擴充**: 
    - **夠用**: 對於功能點相對獨立、狀態交互不複雜的新需求（例如增加新的控制選項、顯示簡單狀態），目前的架構可以支持，可以通過修改或擴展現有服務和組件來實現。
- **複雜擴充**: 
    - **挑戰較大**: 如果未來需要實現涉及多個服務狀態聯動、複雜副作用管理、精細性能優化或需要清晰狀態邊界的功能（如頭部/身體動畫分離），現有架構將面臨挑戰。
    - **痛點**: 狀態分散導致數據流追蹤困難；服務間可能存在隱式耦合；跨服務的狀態同步邏輯容易變得複雜且難以維護。

### 3. 長遠利弊分析 (停止 Zustand 遷移)

- **利 (Pros)**:
    - **節省當前開發時間**: 避免了 Zustand 學習曲線和重構的複雜性。
    - **降低短期風險**: 維持了現有相對穩定的代碼基礎。
    - **可聚焦其他優先級**: 如果有更緊急的功能或優化，可以先處理。
- **弊 (Cons)**:
    - **技術債累積**: 分散的狀態管理模式會增加長期維護成本。
    - **開發效率瓶頸**: 隨著功能增加，狀態管理複雜度會指數級上升，拖慢開發速度。
    - **潛在性能問題**: 未經優化的狀態更新機制可能在高負載下導致性能問題。
    - **團隊協作障礙**: 新成員理解分散狀態的成本更高。
    - **阻礙高級功能實現**: 難以支持需要清晰、集中狀態管理的複雜功能（如精細動畫控制）。

### 4. Zustand 實現困難點分析 (可能原因)

之前的 Zustand 遷移嘗試未能成功，可能的原因包括：

- **狀態依賴複雜性**: 未能完全理清各 Slice 狀態間的複雜依賴關係，導致狀態更新不一致或死循環。
- **服務邏輯與 Store 邊界模糊**: 難以決定哪些邏輯應保留在服務中，哪些應移入 Store Actions，導致實現混亂。
- **高頻狀態處理不當**: 對 `morphTargets` 等高頻更新，未能找到合適的性能優化策略（如 Store 更新節流/防抖，或結合 `useFrame`），直接寫入 Store 導致性能下降或卡頓。
- **異步流程處理複雜**: 對於 WebSocket 消息、API 調用等異步操作，未能很好地將其結果和狀態（加載中/錯誤）整合進 Zustand 的更新流程。
- **漸進遷移的副作用**: 在遷移過程中，新舊狀態管理機制並存，可能導致了難以追蹤的數據不同步或邏輯衝突。
- **心智模型轉換成本**: 從命令式（調用服務方法、依賴回調）到聲明式（更新 Store、由 Store 驅動變化）的轉換需要時間適應。

**結論**: 暫停 Zustand 遷移是當前的務實選擇。我們可以利用現有架構繼續開發或優化其他部分。但長遠來看，集中式狀態管理（無論是 Zustand 還是其他方案）對於提升項目的可維護性、可擴展性和性能仍然是重要的方向。未來可以考慮在團隊對相關技術更熟悉、或項目需求更明確時，再次嘗試引入。
