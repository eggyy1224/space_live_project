...
class WebSocketService {
  ...
  private directorHandler = (payload: Partial<DirectorState>) => {
    this.sendMessage({
      type: "director-state",
      payload,
    } as DirectorStateMessage);
  };

  // 添加防抖動和批量處理的函數
  private _debounceMap: Map<
    string,
    {
      timeout: number | null;
      lastData: any;
  ...
  public connect(): void {
    ...
    if (this.ws) {
      logger.debug(
        `WebSocket object exists. Current state: ${this.ws.readyState}`,
      ...
      if (
        ...
        logger.warn(
          `WebSocket exists but state is ${this.ws.readyState}. Proceeding to reconnect.`,
          LogCategory.WEBSOCKET,
        );
        try {
          this.ws.close();
        } catch (e) {
          /* ignore potential errors closing already closed socket */
        }
        this.ws = null;
      }
    } else {
      logger.debug(
        "WebSocket object (this.ws) is null. Proceeding to create.",
        LogCategory.WEBSOCKET,
      );
    }
    ...
    if (this.reconnectTimer) {
      logger.debug(
        "Clearing existing reconnect timer before new connection attempt.",
        LogCategory.WEBSOCKET,
      );
    ...
    try {
      logger.info(
        {
          msg: "Attempting to create new WebSocket object...",
          details: { url: WS_URL },
        },
        LogCategory.WEBSOCKET,
      );
      ...
      logger.debug(
        {
          msg: "New WebSocket object created.",
          details: { readyState: this.ws.readyState },
        },
        LogCategory.WEBSOCKET,
      );
      logger.debug(
        "Attaching WebSocket event handlers (onopen, onmessage, onerror, onclose)",
        LogCategory.WEBSOCKET,
      );
      ...
      logger.error(
        "Error during WebSocket object creation or handler attachment:",
        LogCategory.WEBSOCKET,
        error,
      );
      this.ws = null;
  ...
  public sendMessage(message: WebSocketMessage): boolean {
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      ...
      logger.debug(
        { msg: `發送 WebSocket 消息: ${message.type}`, details: message },
        LogCategory.WEBSOCKET,
        message.type,
      );
      return true;
  ...
  private handleMessage(event: MessageEvent): void {
    try {
      ...
      logger.debug(
        `[WebSocketService] Calling setLastJsonMessage with data (type: ${data.type})`,
        LogCategory.WEBSOCKET,
      );
      ...
      if (data.type === "camera-angle") {
        ...
        // --- 添加日誌記錄 ---
        console.log(
          "[WebSocketService] Received Emotional Trajectory:",
  ...
  private handleReconnect(): void {
    if (this.retryCount >= WS_RETRY_MAX) {
      logger.warn(
        `嘗試重連已達最大次數 (${WS_RETRY_MAX})，不再嘗試重連`,
        LogCategory.WEBSOCKET,
      );
      return;
    }
    ...
    logger.info(
      `計劃在 ${delay}ms 後進行第 ${this.retryCount} 次重連`,
      LogCategory.WEBSOCKET,
    );
