...
class CharacterControlAgent:
    ...
    def _parse_character_request(self, request: str) -> dict:
        ...
        # 若有『瑜伽』或『瑜珈』關鍵字，隨機選一個瑜珈動作（優先處理）
        if "瑜伽" in request or "瑜珈" in request:
            import random
            yoga_animations = [f"瑜珈動作{i}" for i in range(1, 21)]
            chosen = random.choice(yoga_animations)
            logger.info(f"🎯 瑜伽關鍵字觸發，隨機選擇: {chosen}")
            return {
                "type": "animation",
                "args": {"animation": chosen, "loop": True, "speed": 1.0}
            }
    ...
    def _parse_animation_request(self, request: str) -> dict:
        ...
        animation_mapping = {
            ...
            "Tpose": "Tpose",
            # 新增太空瑜伽系列
            "瑜珈動作1": "瑜珈動作1", "瑜珈動作2": "瑜珈動作2", "瑜珈動作3": "瑜珈動作3", "瑜珈動作4": "瑜珈動作4",
            "瑜珈動作5": "瑜珈動作5", "瑜珈動作6": "瑜珈動作6", "瑜珈動作7": "瑜珈動作7", "瑜珈動作8": "瑜珈動作8",
            "瑜珈動作9": "瑜珈動作9", "瑜珈動作10": "瑜珈動作10", "瑜珈動作11": "瑜珈動作11", "瑜珈動作12": "瑜珈動作12",
            "瑜珈動作13": "瑜珈動作13", "瑜珈動作14": "瑜珈動作14", "瑜珈動作15": "瑜珈動作15", "瑜珈動作16": "瑜珈動作16",
            "瑜珈動作17": "瑜珈動作17", "瑜珈動作18": "瑜珈動作18", "瑜珈動作19": "瑜珈動作19", "瑜珈動作20": "瑜珈動作20"
        }
