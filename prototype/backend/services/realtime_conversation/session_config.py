...
def get_tools_config() -> list:
    ...
    return [
        ...
        },
        {
            ...
            "description": "🎭 智能角色控制工具！統一控制角色的各種外觀和動作，包含大小縮放、胖瘦調整、動畫表演等！💡 重要：必須完整保留用戶的具體數字和精確描述，不可簡化或概括！例如用戶說「調到15倍」就要傳遞「調到15倍」，不能改為「讓角色變大」！",
            "parameters": {
                "type": "object",
                "properties": {
                    "request": {
                        "type": "string",
                        "description": "🎯 角色控制請求描述 - 必須完整保留用戶的原始表達！包含所有數字、倍數、具體要求！如：'身體調整到15倍'、'放大到10倍'、'角色跳舞'、'讓角色變胖一點'、'開始漂浮動畫'等。絕對不可省略數字或簡化描述！"
                    }
                },
                "required": ["request"]
            }
